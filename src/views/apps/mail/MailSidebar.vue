...
<template>
    <div>
        <Button @click="navigate" label="Compose New" class="mb-8 w-full" outlined />
        <div class="overflow-auto">
            <ul class="flex flex-row md:flex-col gap-1 md:gap-2 list-none m-0 p-0 overflow-auto">
                <li
                    ...
                    :class="{ 'bg-primary text-primary-contrast': router.currentRoute.value.path === item.routerLink, 'hover:surface-hover': router.currentRoute.value.path !== item.routerLink }"
                    @click="navigate(item)"
                >
                    <i class="md:mr-4 duration-150 text-lg" :class="item.icon" />
                    <span class="font-medium hidden md:inline">{{ item.label }}</span>
